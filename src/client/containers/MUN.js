...
import {
  ...
  Unmoderated,
  PrimarySpeakersList,
  SecondarySpeakersList,
  SingleSpeaker
} from "./components/SpeakingSession";
...
const MUNConnector = withRouter(
  connect(
    ...
    (dispatch, ownProps) => ({
      toggleCollapse: val =>
        dispatch(toggleCollapse(val, ownProps.match.params.id)),
      setCommitteeName: name =>
        dispatch(setCommitteeName(name, ownProps.match.params.id)),
      setSession: session =>
        dispatch(setSession(session, ownProps.match.params.id)),
      clearSpeakingData: () =>
        dispatch(clearSpeakingData(ownProps.match.params.id))
    })
  )(MUN)
);

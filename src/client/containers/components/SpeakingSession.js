      ...
      <div className="content">
        ...
        <Grid container spacing={16}>
          <Grid item xs={3}>
            <div>
              <span className="button-container">
                <Button
                  onClick={() => this.setState({ paused: !data.paused })}
                  variant="fab"
                  color="primary"
                  aria-label="Add"
                >
          ...
          <Grid item xs={isCode("U") ? 9 : 6}>
            ...
            <div className={shouldDisplay("MPS")}>
              <div className="timer">
                {formatMillis(Math.min(speakingTimer, 1000 * speakingTotal)) +
                  "/" +
                  formatMillis(1000 * speakingTotal)}
              </div>
            ...
            <div className={shouldDisplay("UM")}>
              ...
              <div className="timer">
                {formatMillis(Math.min(timer, 1000 * total)) +
                  "/" +
                  formatMillis(1000 * total)}
              </div>
            </div>
          </Grid>
          ...
          <Grid item xs={3} className={shouldDisplay("MPS")}>
            <div className={shouldDisplay("M")}>
              <Select
                ...
                onChange={e =>
                  this.setState({ currentSpeaker: e.value, speakingTimer: 0 })
                }
              />
            </div>
...
const createType = name => {
  return withRouter(
    connect(
      ...
      (dispatch, ownProps) => ({
        changeState: delta =>
          dispatch(changeSessionData(name, delta, ownProps.match.params.id)),
        updateItem: (item, delta) =>
          dispatch(updateItem(item, delta, ownProps.match.params.id))
      })
    )(SpeakingSession)
  );
};

                ...
                return (
                  <tr key={name}>
                    <td
                      className="hover-red"
                      onClick={() => {
                        if (!this.state.validate || confirm(`Remove ${name}?`))
                          removeDelegate(name);
                      }}
                    >
                      {name}
                    ...
                    <td
                      onClick={() => setPresent(name, getStatus("PRESENT"))}
                      className={classNames({
                        toggled:
                          presentStatus === "PRESENT" ||
                          presentStatus === "PRESENT_VOTING"
                      })}
                    />
                    <td
                      onClick={() =>
...
const RollCallConnector = withRouter(
  connect(
    (state, ownProps) => ({
    ...
    (dispatch, ownProps) => ({
      setPresent: (name, status) =>
        dispatch(setPresent(name, status, ownProps.match.params.id)),
      removeDelegate: name =>
        dispatch(removeDelegate(name, ownProps.match.params.id)),

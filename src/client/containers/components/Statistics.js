...
class Statistics extends React.Component {
  ...
  render() {
    ...
    const totalSpeakingTime = obj =>
      displayed.map(name => obj[name]).reduce((a, b) => a + b);
    const total = Object.keys(stats)
      .map(key => stats[key])
      .map(totalSpeakingTime)
      .reduce((a, b) => a + b, 0);

    const sortedByTime = Object.keys(stats).sort(
      (a, b) =>
        this.state.gToL ^
        (totalSpeakingTime(stats[a]) > totalSpeakingTime(stats[b]))
    );
    return (
      <div>
        <div className="content" style={{ overflowY: "auto" }}>
          ...
          {sortedByTime.length === 0 || total === 0 ? (
            <Typography component="p">
              Looks like nobody has spoke yet!
            </Typography>
          ) : (
        ...
        <Button
          variant="contained"
          onClick={() => confirm("Reset all data?") && reset()}
        >
          Reset
        </Button>
      </div>

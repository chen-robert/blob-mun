...
const applyToState = (state, action) => {
  switch (action.type) {
    ...
    case "CHANGE_SESS":
      ...
      switch (action.delta.type) {
        case "UPDATE_TIMER":
          delta = {
            ...
            speakingTimer:
              state[action.name].speakingTimer + action.delta.granularity
          };

          const currentRoom = state[action.name];
          newSpeakingStats = speakingStats.applyDelta(
            state.speakingStats,
            currentRoom.currentSpeaker,
            action.name,
            action.delta.granularity
          );
          break;
      }
      ...
      return {
        ...state,
        [action.name]: newSessData,
        speakingStats: newSpeakingStats
      };
    case "UPDATE_ITEM":
      let ret = state;
      rooms.forEach(room => {
